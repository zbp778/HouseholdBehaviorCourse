...
class A2Model(EconModelClass):
    ...
    def allocate(self):
        ...
        sim.taxrev = np.nan + np.zeros(shape)
    ...
    def util(self,hours1,hours2,capital1,capital2): #Changed in 1.
        par = self.par

        cons = self.consumption(hours1,hours2,capital1,capital2)

        util_cons = (cons/2)**(1.0+par.eta) / (1.0+par.eta) 
        util_hours1 = par.mu*par.rho_1*(hours1)**(1.0+par.gamma) / (1.0+par.gamma) 
        util_hours2 = (1-par.mu)*par.rho_2*(hours2)**(1.0+par.gamma) / (1.0+par.gamma)

        return util_cons - util_hours1 - util_hours2

    def value(self,period,approximation=True):
        ...
        # b. looking up capital states and optimal hours
        hours1 = sim.h1[0,period]
        hours2 = sim.h2[0,period]
        capital1 = sim.k1[0,period]
        capital2 = sim.k2[0,period]
        ...
        if approximation==True:
            ...
            k1_next = (1.0-par.delta)*capital1 + hours1
            k2_next = (1.0-par.delta)*capital2 + hours2
            V_next = interp_2d(par.k_grid,par.k_grid,sol.V[period+1,:,:],k1_next,k2_next)
            # e. return value
            return util + par.beta*V_next
        else:
            # calculate the expected value instead by starting in the last period, calculating value function and then
            # loop forward iteratively updating V_next
            V_next = 0
            for t in reversed(range(par.T)):
                # i. looking up capital states and optimal hours
                hours1 = sim.h1[0,t]
                hours2 = sim.h2[0,t]
                capital1 = sim.k1[0,t]
                capital2 = sim.k2[0,t]
    ...
    def simulate(self):
        ...
        for i in range(par.simN):
            ...
            for t in range(par.simT):
                ...
                if par.joint_tax:
                    sim.taxrev[i,t] = self.tax_func(sim.income1[i,t] + sim.income2[i,t])
                else:
                    sim.taxrev[i,t] = self.tax_func(sim.income1[i,t]) + self.tax_func(sim.income2[i,t])
